...
(function(window){
    ...
    function lignePaginate(){
        ...
        var pageButtons = function(numberOfPage,currentPage) {
             ...
             **/
            let buttons = "<input type='button' value=' → السابق' class='paginate_control_prev ' onclick='paginate.sort("+(currentPage - 1)+")' "+prevDisabled+">";
            let buttonNumberOfPage = "<input type='button' value='" + currentPage + ' - ' + numberOfPage + "' abled>";
            ...
            let nextButton = "<input type='button' value='التالي ← ' class='paginate_control_next' onclick='paginate.sort("+(currentPage + 1)+")' "+nextDisabled+">";
            buttons +=  nextButton;
